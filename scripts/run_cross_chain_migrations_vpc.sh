#!/bin/bash
# Run Alembic migrations for Cross-Chain Integration via Cloud Run Job (with VPC access)
# This uses Cloud Run Job which has automatic VPC connector access

set -e

PROJECT_ID=${1:-${GCP_PROJECT:-"fusion-prime"}}
SERVICE_NAME="cross-chain-integration-service"
REGION="us-central1"
JOB_NAME="cross-chain-migrations"

echo "🗄️  Running Cross-Chain Integration Database Migrations (VPC Mode)"
...
if ! gcloud run services describe "$SERVICE_NAME" --region="$REGION" --project="$PROJECT_ID" &>/dev/null; then
    ...
    cd "$(dirname "$0")/../services/cross-chain-integration"
...
if gcloud run jobs describe "$JOB_NAME" --region="$REGION" --project="$PROJECT_ID" &>/dev/null; then
    ...
    gcloud run jobs update "$JOB_NAME" \
        ...
        --region="$REGION" \
        --project="$PROJECT_ID" \
        --set-secrets="DATABASE_URL=fp-cross-chain-db-connection-string:latest" \
        --set-cloudsql-instances="fusion-prime:us-central1:fp-cross-chain-db-0c277aa9" \
        --vpc-connector="fusion-prime-connector" \
        --vpc-egress="private-ranges-only" \
        --command="/app/scripts/run_migration.sh" \
        --max-retries=1 \
        --task-timeout=600 \
        --memory=512Mi \
        --cpu=1 || {
...
echo "  3. Deploy remaining Sprint 04 services"
