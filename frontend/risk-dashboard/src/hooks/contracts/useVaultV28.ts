...
import CrossChainVaultV28ABI from '@/abis/CrossChainVaultV28.json';

/**
 * Get the V28 vault address for the current chain
 */
export function getVaultV28Address(chainId: number): Address {
  if (chainId === sepolia.id) {
    return '0xD2652bb55eB6CeeD43d4f0A987f915920d6f7047' as Address;
  } else if (chainId === polygonAmoy.id) {
    return '0xC5a714fe832383E052DbEE841a0Bfd23eEC847cB' as Address;
  }
  return '0xD2652bb55eB6CeeD43d4f0A987f915920d6f7047' as Address;
}
...
export function useSupplyLiquidity(chainId: number = sepolia.id) {
  ...
  const supply = (amount: bigint, gasAmount: bigint = parseEther('0.01')) => {
    const vaultAddress = getVaultV28Address(chainId);

    writeContract({
      address: vaultAddress,
      abi: CrossChainVaultV28ABI,
      functionName: 'supply',
      args: [gasAmount],
