...
import CrossChainVaultV25ABI from '@/abis/CrossChainVaultV25.json';

/**
 * Get the V25 vault address for the current chain
 */
export function getVaultV25Address(chainId: number): Address {
  if (chainId === sepolia.id) {
    return '0x477f54284367CF31B2B7f6BB2Ca4291D3f43a312' as Address;
  } else if (chainId === polygonAmoy.id) {
    return '0xbafd9d789f96d18cedd057899a4ba3273c9f6d0e' as Address;
  }
  return '0x477f54284367CF31B2B7f6BB2Ca4291D3f43a312' as Address;
}
...
export function useSupplyLiquidity(chainId: number = sepolia.id) {
  ...
  const supply = (amount: bigint, gasAmount: bigint = parseEther('0.01')) => {
    const vaultAddress = getVaultV25Address(chainId);

    writeContract({
      address: vaultAddress,
      abi: CrossChainVaultV25ABI,
      functionName: 'supply',
      args: [gasAmount],
