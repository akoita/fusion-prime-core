...
contract BridgeManagerTest is Test {
    ...
    function testEstimateGas() public {
        ...
        bytes memory payload = abi.encode("test", "data");
        (uint256 estimatedGas, string memory protocol) = bridgeManager
            .estimateGas("polygon", payload);

        assertGt(estimatedGas, 0);
        // Protocol may be returned or empty depending on implementation
        // Just verify estimateGas works, protocol is optional
        assertTrue(
            bytes(protocol).length == 0 ||
                keccak256(bytes(protocol)) == keccak256(bytes("axelar"))
        );
    }
    ...
    function test_RevertWhen_NoAdapterForChain() public {
        bridgeManager.registerAdapter(axelarAdapter);
