...
/// @title CrossChainVaultV28
/// @notice Unified vault with cross-chain liquidity synchronization via MessageBridge
/// @dev Uses homemade MessageBridge instead of Axelar/CCIP for cross-chain communication
contract CrossChainVaultV28 is AxelarExecutable {
    using StringAddressUtils for string;

    // State
    IMessageBridge public messageBridge;
    BridgeManager public bridgeManager; // Legacy, kept for compatibility
    ...
    event LiquiditySyncSent(bytes32 indexed messageId, string destinationChain, string sourceChain, uint256 amount, bool isSupply);
    event LiquiditySyncReceived(string sourceChain, uint256 amount, bool isSupply);
    ...
    function supply(uint256 gasAmount) external payable {
        ...
        // Broadcast liquidity update to other chains via MessageBridge
        _broadcastLiquiditySync(thisChainName, supplyAmount, true);
    ...
    function withdrawSupplied(uint256 amount, uint256 gasAmount) external payable {
        ...
        // Broadcast liquidity update to other chains via MessageBridge
        _broadcastLiquiditySync(thisChainName, amount, false);
    ...
    /// @notice Broadcast liquidity update to other chains via MessageBridge
    /// @param sourceChain Chain where liquidity was updated
    /// @param amount Amount supplied or withdrawn
    /// @param isSupply True for supply, false for withdraw
    function _broadcastLiquiditySync(
        string memory sourceChain,
        uint256 amount,
        bool isSupply
    ) internal {
        // Encode the liquidity sync message
        bytes memory payload = abi.encodeWithSignature(
            "handleLiquiditySync(string,uint256,bool)",
            sourceChain,
            amount,
            isSupply
        );

        // Send to each other supported chain
        for (uint256 i = 0; i < allSupportedChains.length; i++) {
            string memory destChain = allSupportedChains[i];

            // Skip this chain and unsupported chains
            if (keccak256(bytes(destChain)) == keccak256(bytes(thisChainName))) {
                continue;
            }

            address destVault = trustedVaults[destChain];
            if (destVault == address(0)) {
                continue; // Skip if no trusted vault configured
            }

            uint64 destChainId = _getChainId(destChain);
            if (destChainId == 0) {
                continue; // Skip if chain ID not found
            }

            // Send message via MessageBridge
            bytes32 messageId = messageBridge.sendMessage(
                destChainId,
                destVault,
                payload
            );

            emit LiquiditySyncSent(messageId, destChain, sourceChain, amount, isSupply);
        }
    }

    /// @notice Handle incoming liquidity sync from another chain
    /// @param sourceChain Chain where liquidity was updated
    /// @param amount Amount supplied or withdrawn
    /// @param isSupply True for supply, false for withdraw
    function handleLiquiditySync(
        ...
        // Only accept messages from MessageBridge relayer
        // In production, add proper access control

        require(supportedChains[sourceChain], "Unsupported source chain");

        // Update local view of remote chain liquidity
        if (isSupply) {
            chainLiquidity[sourceChain] += amount;
        } else {
            if (chainLiquidity[sourceChain] >= amount) {
                chainLiquidity[sourceChain] -= amount;
            }
        }

        emit LiquiditySyncReceived(sourceChain, amount, isSupply);
        emit LiquidityUpdated(sourceChain, chainLiquidity[sourceChain], chainUtilized[sourceChain]);
    }

    /// @notice Get chain ID from chain name
    function _getChainId(string memory chainName) internal pure returns (uint64) {
        bytes32 nameHash = keccak256(bytes(chainName));
        if (nameHash == keccak256(bytes("ethereum"))) return 11155111; // Sepolia
        if (nameHash == keccak256(bytes("polygon"))) return 80002; // Amoy
        return 0;
    }
