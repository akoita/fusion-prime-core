...
import "../src/CrossChainVaultV28.sol";

/**
 * @title DeployVaultV28
 * @notice Deploys CrossChainVaultV28 with MessageBridge for cross-chain liquidity sync
 * @dev Usage:
 *   PRIVATE_KEY=<key> forge script script/DeployVaultV28.s.sol:DeployVaultV28 \
 *     --rpc-url <rpc-url> --broadcast
 */
contract DeployVaultV28 is Script {
    // MessageBridge addresses (from running relayer)
    address constant SEPOLIA_MESSAGE_BRIDGE = 0xA8d853C5b945924d217Ec4119E9f0e2eFf714B8a;
    address constant AMOY_MESSAGE_BRIDGE = 0xEEcd2114162D577c4668B8e92a6FB34d0eA6A149;

    // BridgeManager addresses (legacy, kept for compatibility)
    ...
    function run() external {
        ...
        console.log("Deploying CrossChainVaultV28 with deployer:", deployer);
        console.log("Chain ID:", block.chainid);

        vm.startBroadcast(deployerPrivateKey);

        CrossChainVaultV28 vault;

        if (block.chainid == 11155111) {
            // Sepolia deployment
            console.log("\n=== Deploying on Sepolia ===");
            console.log("MessageBridge:", SEPOLIA_MESSAGE_BRIDGE);
            ...
            vault = new CrossChainVaultV28(
                SEPOLIA_MESSAGE_BRIDGE,
            ...
            console.log("CrossChainVaultV28:", address(vault));
            ...
            // Amoy deployment
            console.log("\n=== Deploying on Amoy ===");
            console.log("MessageBridge:", AMOY_MESSAGE_BRIDGE);
        ...
        console.log("\n7. Test Cross-Chain Liquidity Sync:");
        console.log("   - Supply on Sepolia, check that Amoy vault sees the liquidity");
        console.log("   - Verify relayer picks up and delivers the liquidity sync message");
        console.log("   - Check: cast call <amoy-vault> 'chainLiquidity(string)(uint256)' 'ethereum'");
    }
}
