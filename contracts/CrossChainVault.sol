...
import {CrossChainVaultV30} from "./CrossChainVaultV30.sol";
...
/**
 * @title CrossChainVaultV31
 * @notice Cross-chain vault with KYC/compliance gating
 * @dev Extends V30 with identity verification via ERC-735 claims
 *
 * Features:
 * - All V30 features (variable/stable rates, flash loans, pause, reserves)
 ...
 */
contract CrossChainVaultV31 is CrossChainVaultV30 {
    ...
    constructor(address _gateway, address _gasService, address _priceOracle, address _interestRateModel)
        CrossChainVaultV30(_gateway, _gasService, _priceOracle, _interestRateModel)
    {
    ...
    function registerIdentity(address identityContract) external {
        ...
        if (userIdentities[msg.sender] != address(0)) revert AlreadyRegistered();

        // Verify it's a valid identity contract by checking if it has the getClaim function
        try IERC735Minimal(identityContract).getClaimIdsByTopic(KYC_VERIFIED) returns (bytes32[] memory) {
    ...
    function removeIdentity() external {
        if (userIdentities[msg.sender] == address(0)) revert IdentityNotRegistered();
     ...
     * @return True if user has all required claims from trusted issuers
     */
     ...
     */
    function hasValidClaim(address identity, uint256 topic) public view returns (bool) {
        try IERC735Minimal(identity).getClaimIdsByTopic(topic) returns (bytes32[] memory claimIds) {
            for (uint256 i = 0; i < claimIds.length; i++) {
                try IERC735Minimal(identity).getClaim(claimIds[i]) returns (
                    uint256, uint256, address issuer, bytes memory, bytes memory, string memory
                ) {
    ...
    function getKYCStatus(address user)
        external
        view
        returns (bool hasIdentity, bool isVerified, uint256[] memory missingClaims)
    {
        address identity = userIdentities[user];
     ...
     */
    function deposit() external payable override whenNotPaused onlyCompliant nonReentrant {
     ...
     */
    function borrowWithMode(address token, uint256 amount, RateMode mode)
        external
        override
        whenNotPaused
        onlyCompliant
