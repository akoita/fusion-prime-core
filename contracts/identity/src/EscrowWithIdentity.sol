...
import {IdentityVerifier} from "./IdentityVerifier.sol";
...
contract EscrowWithIdentity {
    ...
    constructor(
        ...
        if (_requiredClaims.length > 0) {
            (, bool payerHasClaims) = IdentityVerifier.verifyMultipleClaims(_identityFactory, _payer, _requiredClaims);
            (, bool payeeHasClaims) = IdentityVerifier.verifyMultipleClaims(_identityFactory, _payee, _requiredClaims);
        ...
        emit EscrowCreated(_payer, _payee, amount, releaseTime, _approvalsRequired, _requiredClaims);
    }
    ...
    function _verifyIdentity(address _address) internal view {
        ...
        if (requiredClaims.length > 0) {
            (, bool hasClaims) = IdentityVerifier.verifyMultipleClaims(identityFactory, _address, requiredClaims);
            if (!hasClaims) {
                revert ClaimVerificationFailed();
            }
        }
    ...
    function canRelease() external view returns (bool) {
        ...
        if (!identityBypassEnabled) {
            if (!IdentityVerifier.hasIdentity(identityFactory, payer)) return false;
            if (!IdentityVerifier.hasIdentity(identityFactory, payee)) return false;

            if (requiredClaims.length > 0) {
                (, bool payerHasClaims) = IdentityVerifier.verifyMultipleClaims(identityFactory, payer, requiredClaims);
                (, bool payeeHasClaims) = IdentityVerifier.verifyMultipleClaims(identityFactory, payee, requiredClaims);
    ...
    {
        return (
            released, refunded, approvalsCount, approvalsRequired, releaseTime, this.canRelease(), identityBypassEnabled
        );
    }
     ...
     */
    function verifyAddress(address _address) external view returns (bool hasIdentity, bool hasClaims) {
        hasIdentity = IdentityVerifier.hasIdentity(identityFactory, _address);

        if (requiredClaims.length > 0) {
            (, hasClaims) = IdentityVerifier.verifyMultipleClaims(identityFactory, _address, requiredClaims);
        } else {
